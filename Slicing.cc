...
inline bool Slicing::isCross(const Vec3d& v1, const Vec3d& v2) const {	//not include at the vertex
...
void Slicing::intersection() {
	...
	for (const auto& t : triangles) {
		if (isCross(t.v1, t.v2)) {
			if (isCross(t.v1, t.v3)) {	//Cross v1--v2 and v1--v3
				intersections.push_back(Cross(Vec2d(t.v1, t.v2, z), Vec2d(t.v1, t.v3, z)));
				continue;
			}
			else if (fabs(z - t.v3.z) < thr) {	//Cross v1--v2 and v3
				intersections.push_back(Cross(Vec2d(t.v1, t.v2, z), Vec2d(t.v3.x, t.v3.y)));
				continue;
			}
			else {	//definitely cross v1--v2 and v2--v3
				intersections.push_back(Cross(Vec2d(t.v1, t.v2, z), Vec2d(t.v2, t.v3, z)));
				continue;
			}
		}
		else if (isCross(t.v1, t.v3)) {
			if (fabs(z - t.v2.z) < thr) {	//Cross v1--v3 and v2
				intersections.push_back(Cross(Vec2d(t.v1, t.v3, z), Vec2d(t.v2.x, t.v2.y)));
				continue;
			}
			else {	//Cross v1--v3 and v2--v3
				intersections.push_back(Cross(Vec2d(t.v1, t.v3, z), Vec2d(t.v2, t.v3, z)));
				continue;
			}
		}
		else if (fabs(z - t.v1.z) < thr) {
			if (isCross(t.v2, t.v3)) {	//Cross v1 and v2--v3
				intersections.push_back(Cross(Vec2d(t.v1.x, t.v1.y), Vec2d(t.v2, t.v3, z)));
				continue;
			}
				...
				if (fabs(z - t.v3.z) < thr)	//Through the whole facet
					...
					if (!isSectionExist(cr))
						intersections.push_back(cr);
					continue;
				}
			}
			else if (fabs(z - t.v3.z) < thr) {	// v1 and v3, still need "if()", because may only cross one vertex(v1)
			...
			if (fabs(z - t.v3.z) < thr) {	//v2 and v3
