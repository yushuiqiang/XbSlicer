...
//After assign the triangles, no need to check triangles which is out of the high range
void Stl::assignTriangles() {
	...
	for (const auto& t : triangles) {
		...
		for (int slice = int((zmint - zmin) * invDz); slice <= int((zmaxt - zmin) * invDz); slice++)
...
void Stl::printSTL(){	//need to comment "triangles.clear();" in assignTriangles function
