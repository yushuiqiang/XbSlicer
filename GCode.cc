...
void Gcode::resetE() {	//Reset E to 0
...
void Gcode::fillLoop(Loop& loop){
	...
	*/
	...
	for (const auto& lp : ploops) {	//assign all intersections to partial
		Vec2d v1, v2(lp.back());
...
void Gcode::fill() {
	...
	for (int i = 1; i < loops.size(); i++) {
		...
		if (minX > loops.at(i).getminX())
			minX = loops.at(i).getminX();
		if (maxX < loops.at(i).getmaxX())
			maxX = loops.at(i).getmaxX();
	}
	...
	for (const auto& lp : loops) {	//assign all intersections to partial
		Vec2d v1(lp.back());
		Vec2d v2(lp.front());
		...
		for (int i = 1; i < lp.size(); i++) {	//from the front to the item before the back
			v1 = v2;
			index1 = index2;
			v2 = lp.at(i);
